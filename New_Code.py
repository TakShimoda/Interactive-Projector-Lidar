...
# Globals
xbound, ybound = 500, 375                               #Screen size
...
def consumer(queue, prev_queue, event):
    ...
    while not queue.empty():
        ...
        for i in data:
            ...
            if x<xbound and y<ybound and 15<intensity<100:
...
#Wrap this function with GUI label change and log
@mouse_wrapper
def mouse(current_data, previous_data):
    ...
    m.moveTo(curent_data[0][0], current_data[0][1])
    if (time_dif) < 0.2 and normdist < 50:
        return('drag')
    else:
        return('tap')
        #print('%d\t%d\t%4.2f'  % (x, y, intensity))
